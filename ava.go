...
func New(opts ...Options) *Service {
	...
	s.server = newServer(s.opts)
	return s
}

func (s *Service) Client() *Client {
	if s.client == nil {
		s.client = newClient(s.opts.Endpoint, s.opts.Registry)
	}
...
func (s *Service) Run() {
	...
	go func() {
		...
		case c := <-ch:
			...
			Infof("received signal %s ,service [%s] exit!", strings.ToUpper(c.String()), s.opts.Name)
