...
// FileReq must equals to github.com/go-ava/ava/parcel/packet/parcel.proto FileReq
...
func NewSayClient(c *ava.Client) SayClient {
	return &sayClient{c}
}
...
func RegisterSayServer(s *ava.Server, h SayServer) {
	var r = &sayHandler{h: h, s: s}
	s.RegisterHandler("/"+s.Name()+"/say/hi", r.Hi)
}
...
func NewSaySrvClient(c *ava.Client) SaySrvClient {
	return &saySrvClient{c}
}
...
func RegisterSaySrvServer(s *ava.Server, h SaySrvServer) {
	var r = &saySrvHandler{h: h, s: s}
	s.RegisterHandler("/"+s.Name()+"/saysrv/say", r.Say)
	s.RegisterStreamHandler("/"+s.Name()+"/saysrv/stream", r.Stream)
	s.RegisterChannelHandler("/"+s.Name()+"/saysrv/channel", r.Channel)
}
...
func NewHttpClient(c *ava.Client) HttpClient {
	return &httpClient{c}
}
...
func RegisterHttpServer(s *ava.Server, h HttpServer) {
	var r = &httpHandler{h: h, s: s}
	s.RegisterHandler("/"+s.Name()+"/http/hi", r.Hi)
	s.RegisterHandler("/"+s.Name()+"/http/upload", r.Upload)
}
